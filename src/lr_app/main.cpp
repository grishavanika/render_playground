...
static void OnWindowResize(GameState& game, float width, float height)
{
    Panic(game.device_);
    ...
    // Set up the viewport.
    game.vp_.Width = width;
    game.vp_.Height = height;
}

// https://learnopengl.com/Getting-started/Camera
static void OnWindowMouseInput(GameState& game, HRAWINPUT handle)
{
    RAWINPUT raw{};
...
static void AddMessageHandling(StubWindow& window, GameState& game)
{
        ...
        , [&game](HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) -> LRESULT
    {
        ...
        OnWindowResize(game, LOWORD(lparam), HIWORD(lparam));
        return ::DefWindowProc(hwnd, message, wparam, lparam);
    });
        ...
        , [&game](HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) -> LRESULT
    {
        ...
        OnWindowMouseInput(game, HRAWINPUT(lparam));
        return ::DefWindowProc(hwnd, message, wparam, lparam);
    });
...
static void ImGui_Setup(GameState& game, StubWindow& window)
{
...
static void ImGui_TweaksInput(GameState& game)
{
...
static void ImGui_ModelInput(GameState& game, RenderModel& render_model, AllKnownShaders& known_shaders)
{
    ImGuiState& imgui = game.imgui_;
...
static void TickImGui(GameState& game, RenderModel& render_model, AllKnownShaders& known_shaders)
{
    ...
    ImGui_TweaksInput(game);
    ImGui_ModelInput(game, render_model, known_shaders);
}
...
int WINAPI _tWinMain(HINSTANCE /*hInstance*/, HINSTANCE /*hPrevInstance*/, LPTSTR /*lpCmdLine*/, int /*nCmdShow*/)
{
    ...
    // Accept WM_DROPFILES.
    ::DragAcceptFiles(window.wnd(), TRUE);
    // Accept raw WM_INPUT.
