...
void* Assimp_ToBlob(const char* filename, size_t& size_)
{
    ...
    offset = 0;
    void* blob_data = malloc(final_size);
    std::uint32_t blob_offset = 0;
    Panic(blob_data);
    auto blob_write = [&](const void* data, std::uint32_t size) -> std::uint32_t
    {
        Panic(blob_offset + size <= final_size);
        std::memcpy(static_cast<std::uint8_t*>(blob_data) + blob_offset, data, size);
        blob_offset += size;
        return size;
    };
    ...
    Panic(blob_write(&header, to_write) == to_write);
    offset += to_write;
    // Meshes.
    to_write = std::uint32_t(sizeof(MeshData) * header.meshes_count);
    Panic(blob_write(meshes.data(), to_write) == to_write);
    offset += to_write;
    // Textures.
    to_write = std::uint32_t(sizeof(TextureData) * header.textures_count);
    Panic(blob_write(textures.data(), to_write) == to_write);
    offset += to_write;
    ...
    for (const AssimpMesh& mesh : model.meshes)
    {
        ...
        Panic(blob_write(mesh.vertices.data(), to_write) == to_write);
        offset += to_write;

        to_write = std::uint32_t(mesh.indices.size() * sizeof(Index));
        Panic(blob_write(mesh.indices.data(), to_write) == to_write);
        offset += to_write;
    }
    ...
    for (AssimpModel::Blob& blob : model.materials)
    {
        ...
        Panic(blob_write(blob.data, to_write) == to_write);
        offset += to_write;
