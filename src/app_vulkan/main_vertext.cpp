...
int WINAPI _tWinMain(HINSTANCE /*hInstance*/, HINSTANCE /*hPrevInstance*/, LPTSTR /*lpCmdLine*/, int /*nCmdShow*/)
{
    ...
    buffer_info.flags = 0;
    buffer_info.size = sizeof(Vertex) * std::size(vertices);
    buffer_info.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    // buffer_info.queueFamilyIndexCount = 
    // buffer_info.pQueueFamilyIndices = 
    VkBuffer vertex_buffer{};
    Panic(vkCreateBuffer(device, &buffer_info, nullptr, &vertex_buffer));
    VkMemoryRequirements mem_requirements{};
    vkGetBufferMemoryRequirements(device, vertex_buffer, &mem_requirements);

    VkPhysicalDeviceMemoryProperties mem_properties{};
    vkGetPhysicalDeviceMemoryProperties(physical_device, &mem_properties);
    constexpr VkMemoryPropertyFlags required_mem_properties =
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    uint32_t mem_index = uint32_t(-1);
    for (uint32_t i = 0; i < mem_properties.memoryTypeCount; ++i)
    {
        if ((mem_requirements.memoryTypeBits & (1 << i))
            && ((mem_properties.memoryTypes[i].propertyFlags & required_mem_properties) == required_mem_properties))
        {
            mem_index = i;
            break;
        }
    ...
    Panic(vkAllocateMemory(device, &vertex_alloc_info, nullptr, &vertex_buffer_memory));
    Panic(vkBindBufferMemory(device, vertex_buffer, vertex_buffer_memory, 0));

    void* data = nullptr;
    Panic(vkMapMemory(device, vertex_buffer_memory, 0, buffer_info.size, 0, &data));
