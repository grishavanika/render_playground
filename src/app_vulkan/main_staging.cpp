...
#include "shaders/shader_no_hardcoded_vertex.vert.spv.h"
...
static VkVertexInputBindingDescription Vertex_BindingDescription()
{
...
static std::array<VkVertexInputAttributeDescription, 2> Vertex_AttributeDescriptions()
{
...
int WINAPI _tWinMain(HINSTANCE /*hInstance*/, HINSTANCE /*hPrevInstance*/, LPTSTR /*lpCmdLine*/, int /*nCmdShow*/)
{
    ...
    VkShaderModule vertex_bytecode   = make_shader(shader_no_hardcoded_vertex_vert_spv, std::size(shader_no_hardcoded_vertex_vert_spv));
    ...
    auto binding_description = Vertex_BindingDescription();
    auto attribute_descriptions = Vertex_AttributeDescriptions();
    ...
    auto allocate_buffer = [&device, &physical_device]
            ...
            , VkBuffer& buffer, VkDeviceMemory& buffer_memory)
    {
        ...
        Panic(vkCreateBuffer(device, &buffer_info, nullptr, &buffer));
        ...
        Panic(vkAllocateMemory(device, &vertex_alloc_info, nullptr, &buffer_memory));
        Panic(vkBindBufferMemory(device, buffer, buffer_memory, 0));
    };

    VkBuffer staging_buffer{};
    VkDeviceMemory staging_buffer_memory{};
    const std::size_t size = sizeof(Vertex) * std::size(vertices);
    ...
    void* data = nullptr;
    Panic(vkMapMemory(device, staging_buffer_memory, 0, size, 0, &data));
    ...
    // TODO: use separate/non-graphics queue, see "Transfer queue":
    // https://vulkan-tutorial.com/Vertex_buffers/Staging_buffer
    auto copy_buffers_sync = [&command_pool, &device, &graphics_queue]
        (VkBuffer src, VkBuffer dst, VkDeviceSize size)
    {
        ...
        Panic(vkAllocateCommandBuffers(device, &alloc_info, &command_buffer));
        VkCommandBufferBeginInfo begin_info{};
        begin_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        begin_info.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;

        Panic(vkBeginCommandBuffer(command_buffer, &begin_info));
        VkBufferCopy copy_region{};
        copy_region.srcOffset = 0;
        copy_region.dstOffset = 0;
        copy_region.size = size;
        vkCmdCopyBuffer(command_buffer, src, dst, 1, &copy_region);
        Panic(vkEndCommandBuffer(command_buffer));
        ...
        fence_info.flags = 0;
        VkFence fence_wait{};
        Panic(vkCreateFence(device, &fence_info, nullptr, &fence_wait));
        Panic(vkQueueSubmit(graphics_queue, 1, &submit_info, fence_wait));
        Panic(vkWaitForFences(device, 1, &fence_wait, VK_TRUE, UINT64_MAX));
        vkDestroyFence(device, fence_wait, nullptr);
        vkFreeCommandBuffers(device, command_pool, 1, &command_buffer);
    };

#if (0) // Note: NOT copying to vertex buffer
        // while still using in vkCmdDraw() draw call below
    copy_buffers_sync(staging_buffer, vertex_buffer, size);
    ...
    for (uint32_t i = 0, count = uint32_t(framebuffers.size()); i < count; ++i)
    {
        ...
        vkCmdDraw(command_buffers[i], uint32_t(std::size(vertices))/*vertexCount*/, 1/*instanceCount*/, 0/*firstVertex*/, 0/*firstInstance*/);
